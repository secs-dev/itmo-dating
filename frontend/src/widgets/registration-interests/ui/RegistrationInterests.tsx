import {List, Multiselect, Placeholder, Rating, Section} from "@telegram-apps/telegram-ui";
...
export const RegistrationInterests = ({changeRD}: RegistrationInterestsProps) => {
    ...
    useEffect(() => {
        setInterests(
            (prevState) => {
                const oldInterests = Array.from(prevState.values()).filter(i => (i.topicId in selected.map(s => s.value)))
                const newSelectedOptionInterests = selected.filter(s => !(Number(s.value) in interests))
                const newSelectedInterests: Interest[] = newSelectedOptionInterests.map(s => {
                    return {
    ...
    return (
        <Section header="Your interests">
            <List>
                <Multiselect header="Topics" value={selected} onChange={setSelected}
                             options={
                                 topics
                                     .filter(_ => selected.length < MAX_INTEREST_COUNT)
                                     .map(topic => {
                        ...
                        (selected.map(topic =>
                            (<Section key={topic.value} header={topic.label}>
                                <Rating
                                    ...
                                    onChange={e => setInterests((prevState) => {
                                        const newMap = new Map(prevState)
                                        newMap.set(Number(topic.value), {topicId: Number(topic.value), level: e})
                                        return newMap
                                    })}
                                    precision={1}/>
                            </Section>)))
                        : <Placeholder description="You did not choose any interests" header="Empty yet">
                            <img
